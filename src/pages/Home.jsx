...
import { Shield, Droplets, Wifi, Leaf, Star, ChevronRight, Zap, Heart, Users } from "lucide-react";
        ...
        <motion.div initial="hidden" animate="visible" variants={fadeUp} transition={{ duration: 0.6 }}>
          ...
          <div className="flex gap-3">
            <Button asChild size="lg" className="bg-primary hover:bg-primary/90 shadow-lg shadow-primary/25">
              <Link to="/product">Mua ngay <ChevronRight className="w-4 h-4 ml-1" /></Link>
            </Button>
      ...
      <div className="max-w-7xl mx-auto px-4 grid md:grid-cols-2 gap-12 items-center">
        ...
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp}>
          ...
          <div className="grid grid-cols-3 gap-4">
            {[{ label: "TDS", value: "005 ppm" }, { label: "pH", value: "7.2" }, { label: "Độ đục", value: "0.1 NTU" }].map((s, i) =>
            <div key={i} className="text-center p-4 bg-muted/50 rounded-xl hidden">
...
function ReviewsSection() {
  ...
  useEffect(() => {
    base44.entities.Review.list("-created_date", 6).then(setReviews).catch(() => {});
  }, []);
    ...
    <section className="py-16 bg-muted/30">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-6">
          {mockReviews.map((r, i) =>
          <motion.div key={r.id || i} initial="hidden" whileInView="visible" viewport={{ once: true }} variants={fadeUp} transition={{ delay: i * 0.1 }} className="bg-white p-6 rounded-2xl shadow-sm">
              <div className="flex items-center gap-1 mb-3">
                {Array.from({ length: 5 }).map((_, j) =>
              <Star key={j} className={`w-4 h-4 ${j < (r.rating || 5) ? "fill-yellow-400 text-yellow-400" : "text-gray-200"}`} />
              )}
              </div>
    ...
    <section className="py-16 bg-primary text-primary-foreground">
      <div className="max-w-3xl mx-auto px-4 text-center">
        ...
        <Button asChild size="lg" variant="secondary" className="shadow-lg">
          <Link to="/product">Đặt hàng ngay - {formatPrice(PRODUCT.price)} <ChevronRight className="w-4 h-4 ml-1" /></Link>
        </Button>
      </div>
    </section>);
